...
1. **Crea il database** su supabase.com (account gratuito, bastano email + password):
   - **New project** -> dai un nome (es. "fantagp") -> scegli una password per il database (annotala) -> crea
...
2. **Clicca il bottone Deploy** qui sotto (richiede un account Vercel, gratuito, puoi crearlo al volo con GitHub):
   ...
   [![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/TUO-USERNAME/fantagp&env=SUPABASE_URL,SUPABASE_SERVICE_ROLE_KEY,JWT_SECRET,CRON_SECRET,SETUP_CODE&envDescription=Valori%20necessari%20per%20far%20funzionare%20FantaGP&envLink=https://github.com/TUO-USERNAME/fantagp/blob/main/README.md)
...
1. Vai su github.com/new, crea un repository (es. "fantagp"), **pubblico** (necessario per il bottone Deploy automatico — se preferisci tenerlo privato, usa l'installazione manuale più sotto, dove questo vincolo non c'è)
2. Sul tuo computer, apri il Terminale (Mac) o PowerShell (Windows) nella cartella del progetto:

```bash
git init
git add .
git commit -m "Prima versione FantaGP"
git branch -M main
git remote add origin https://github.com/TUO-USERNAME/fantagp.git
...
FantaGP è una **Progressive Web App (PWA)**: si installa come un'app
...
- **Android/Chrome/desktop**: dopo il login compare un banner in basso
  "Installa FantaGP" — un tocco e l'icona appare sulla Home/Dock
