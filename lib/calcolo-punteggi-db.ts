...
export async function ricalcolaPunteggiEvento(eventoId: string): Promise<void> {
  ...
  // Se la gara è ripartita dopo bandiera rossa esistono sia 'RAC' (interrotta)
  // che 'RAC2' (la ripartenza, valida ai fini punteggio): preferiamo sempre
  // RAC2 quando presente, altrimenti usiamo RAC.
  function sessioneGaraValida(categoriaId: string | undefined): string | undefined {
    if (!categoriaId) return undefined;
    const rac2 = sessioni?.find((s) => s.categoria_id === categoriaId && s.tipo === 'RAC2');
    if (rac2) return rac2.id;
    return sessioni?.find((s) => s.categoria_id === categoriaId && s.tipo === 'RAC')?.id;
  }

  const sessioneRacMoto3 = sessioneGaraValida(catMoto3);
  const sessioneRacMoto2 = sessioneGaraValida(catMoto2);
  const sessioneRacMotoGp = sessioneGaraValida(catMotoGp);
