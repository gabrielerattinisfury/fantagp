...
export interface ApiEvento {
  ...
  test?: boolean; // presente su alcune risposte per i test pre-stagionali
}
...
/**
 * Normalizza il nome categoria restituito dall'API MotoGP. L'endpoint
 * /results/categories restituisce i nomi con il simbolo di marchio
 * registrato (es. "MotoGP™", "Moto2™"), che il nostro DB NON usa
 * (colonna motogp_categorie.codice = 'MotoGP', 'Moto2', 'Moto3' senza ™).
 * Qualsiasi confronto/lookup su questo nome DEVE passare da qui, altrimenti
 * il match fallisce silenziosamente (es. sincronizzaSessioni che non trova
 * mai la categoria e salta tutte le sessioni senza errore visibile).
 */
export function normalizzaNomeCategoria(nomeApi: string): 'MotoGP' | 'Moto2' | 'Moto3' | null {
  const pulito = nomeApi.replace(/™/g, '').trim();
  if (pulito === 'MotoGP') return 'MotoGP';
  if (pulito === 'Moto2') return 'Moto2';
  if (pulito === 'Moto3') return 'Moto3';
  return null; // altre categorie (es. MotoE) non gestite dal fantagame
}
 ...
 *
 * NOTA su RAC2: l'API non ha un tipo dedicato per la gara ripartita dopo
 * bandiera rossa - restituisce semplicemente una seconda sessione con lo
 * stesso type "RAC". La distinzione RAC/RAC2 va quindi fatta a monte, in
 * sincronizzaSessioni(), confrontando gli orari delle sessioni dello stesso
 * evento/categoria: qui il tipo grezzo resta sempre 'RAC'.
 */
