    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-1">
          ...
          <Link
            href="/classifica/categoria"
            className="text-sm text-asfalto-300 hover:text-white flex items-center gap-1.5 px-3 py-1.5 rounded-xl hover:bg-white/5 transition-colors"
          >
            Per categoria <i className="ti ti-chevron-right text-base" aria-hidden="true" />
          </Link>
        </div>
      </div>
        ...
        <ol className="space-y-2">
          {classifica.map((riga, indice) => {
            ...
            return (
              <li
                key={riga.utente_id}
                className={`glass-hover flex items-center gap-3 rounded-2xl px-4 py-3.5 ${
                  sonIo
                    ? 'glass-strong border-bandiera-giallo/40'
                    : posizione <= 3
                    ? 'glass-strong'
                    : 'glass'
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate flex items-center gap-1.5">
                    ...
                    <StelleMondiali n={riga.stelle_mondiali} />
                  </p>
