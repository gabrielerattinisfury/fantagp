...
import { StelleMondiali } from '@/components/stelle-mondiali';
...
function PaginaAdminInterna() {
  ...
  // L'account admin compete anch'esso in classifica (vedi vista_classifica_generale),
  // quindi ha una sua rosa da gestire — ma non compare nella lista "Fantamotociclisti"
  // qui sotto (quella è per gestire GLI ALTRI). Gli serve un punto d'accesso dedicato
  // e sempre visibile alla propria rosa, altrimenti resta irraggiungibile da UI.
  const propriaScheda = dati.utenti.find((u) => u.id === utente?.id);
  ...
  return (
    <div className="space-y-8">
        ...
        <section className="glass rounded-2xl p-5">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3 min-w-0">
              <span
                className="numero-gara text-base w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
                style={{ color: propriaScheda.colore_primario, backgroundColor: `${propriaScheda.colore_primario}22` }}
              >
                {propriaScheda.numero_gara ?? '–'}
              </span>
              <div className="min-w-0">
                <h2 className="font-display font-semibold flex items-center gap-1.5">
                  La tua rosa
                  <StelleMondiali n={propriaScheda.stelle_mondiali} />
                </h2>
                <p className="text-xs text-asfalto-500 truncate">
                  Anche tu, {propriaScheda.nome_squadra}, competi in classifica: gestisci qui i tuoi 9 piloti.
                </p>
              </div>
            </div>
            <Link
              href={`/admin/modifica-utente?utenteId=${propriaScheda.id}`}
              className="text-sm bg-bandiera-rosso hover:bg-red-600 transition-colors text-white font-medium rounded-lg px-4 py-2 flex items-center gap-2 shrink-0"
            >
              <i className="ti ti-edit" aria-hidden="true" />
              Modifica la mia rosa
            </Link>
          </div>
        </section>
      )}
      ...
      {/* Fantamotociclisti (gli altri giocatori della lega, non tu) */}
      <section className="glass rounded-2xl p-5">
        <div className="flex items-center justify-between gap-3 mb-4">
          <h2 className="font-display font-semibold">Fantamotociclisti ({dati.utenti.filter((u) => u.ruolo === 'utente').length})</h2>
          <Link
        ...
        <ul className="space-y-2">
          {dati.utenti
            ...
            .map((u) => (
              <li
                key={u.id}
                className="glass-hover flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3"
              >
                <span
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate flex items-center gap-1.5">
                    {u.nome_squadra}
                    <StelleMondiali n={u.stelle_mondiali} />
                  </p>
                ...
                <Link
                  href={`/admin/modifica-utente?utenteId=${u.id}`}
