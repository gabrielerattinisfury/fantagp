...
export function CountdownDeadline({
  ...
  if (!deadline) {
    return (
      <div className="glass rounded-2xl px-4 py-3 text-sm text-asfalto-300 flex items-center gap-2">
  ...
  return (
    <div
      ...
      className={`rounded-2xl px-4 py-3.5 border backdrop-blur-md shadow-lg ${
        chiuso
          ? 'bg-asfalto-800/80 border-white/10 text-asfalto-400 shadow-black/20'
          : urgente
          ? 'bg-bandiera-rosso/15 border-bandiera-rosso/40 text-white shadow-bandiera-rosso/10'
          : 'bg-bandiera-giallo/10 border-bandiera-giallo/30 text-white shadow-bandiera-giallo/10'
      }`}
    >
