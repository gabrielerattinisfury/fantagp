...
export async function sincronizzaCalendario(): Promise<RisultatoSync> {
  ...
  try {
    ...
    let aggiornati = 0;
    // Il numero di round conta solo i weekend di gara veri: i test
    // pre-stagionali sono esclusi dalla numerazione (restano numero_round =
    // null), altrimenti "Round 1" mostrato in UI sarebbe in realtà un test.
    let prossimoRound = 1;

    for (let i = 0; i < eventiOrdinati.length; i++) {
      const ev = eventiOrdinati[i];
      const annullato = Boolean(ev.cancelled) || ev.status === 'CANCELLED';
      // I test pre-stagionali (es. "Sepang Test", "Valencia Test") non sono
      // weekend di gara: l'API non ha sempre un campo booleano dedicato e
      // affidabile, quindi come euristica di sicurezza controlliamo anche il
      // nome evento. Se l'API espone `test` lo rispettiamo per primo.
      const eTest = Boolean(ev.test) || /\btest\b/i.test(ev.name ?? '') || /\btest\b/i.test(ev.sponsored_name ?? '');
      const numeroRound = eTest ? null : prossimoRound++;
  ...
  try {
    ...
    for (const evento of eventi) {
      for (const catApi of categorieApi) {
        // L'API restituisce i nomi categoria col simbolo ™ (es. "MotoGP™"),
        // che il nostro DB non usa: senza questa normalizzazione il lookup
        // sotto falliva sempre silenziosamente e NESSUNA sessione veniva mai
        // sincronizzata (deadline/countdown quindi mai affidabili).
        const codiceCategoria = normalizzaNomeCategoria(catApi.name);
        if (!codiceCategoria) continue; // categoria non riconosciuta (es. MotoE), ignorata
        ...
        // Gara ripartita dopo bandiera rossa: l'API espone due sessioni
        // distinte con lo stesso type "RAC". Ordinando per orario, la prima
        // resta 'RAC' (è anche quella usata come deadline schieramento per
        // Moto3, vedi lib/deadline.ts) e le successive diventano 'RAC2', così
        // calcolo-punteggi-db.ts può preferire la gara valida (RAC2) su
        // quella interrotta (RAC) senza ambiguità.
        const sessioniOrdinate = [...sessioniApi].sort(
          (a, b) => new Date(a.date ?? 0).getTime() - new Date(b.date ?? 0).getTime()
        );
        let gareRacViste = 0;

        for (const sess of sessioniOrdinate) {
          let tipo = normalizzaTipoSessione(sess.type);
          if (tipo === 'RAC') {
            gareRacViste++;
            if (gareRacViste >= 2) tipo = 'RAC2';
          }
          const tipiValidi = ['FP1','FP2','FP3','FP4','PR','Q1','Q2','SPR','RAC','RAC2','WUP'];
