...
create table motogp_eventi (
  ...
  -- true per i test pre-stagionali (es. "Sepang Test", "Shakedown Test"): non
  -- sono weekend di gara e vanno esclusi dal calendario fanta e dal computo
  -- round. Vedi lib/sync-motogp.ts per come viene dedotto in sincronizzazione.
  e_test boolean not null default false,
...
create table motogp_sessioni (
  ...
  -- SPR = Sprint Race, RAC = Gara principale, RAC2 = ripartenza dopo bandiera
  -- rossa (vedi lib/sync-motogp.ts): quando presente ha la precedenza su RAC
  -- ai fini del calcolo punteggi (lib/calcolo-punteggi-db.ts)
...
create table fanta_utenti (
  ...
  attivo boolean not null default true,
  -- Stelle mondiali: riconoscimento onorifico assegnato manualmente
  -- dall'admin (PATCH /api/admin/utenti/stelle), es. per titoli vinti in
  -- stagioni precedenti. Puramente decorativo, non entra nel calcolo punti.
  stelle_mondiali integer not null default 0,
  creato_il timestamptz not null default now(),
  aggiornato_il timestamptz not null default now()
);
...
create table fanta_rose (
  ...
  pilota_id uuid not null references motogp_piloti(id),
  categoria_id uuid not null references motogp_categorie(id),
  -- "slot" identifica la casella di rosa (es. moto3_a, moto3_b, moto3_c,
  -- moto2_a, moto2_b, moto2_c, motogp_a, motogp_b, motogp_c). Permette di
  -- sapere quale pilota ha sostituito quale, mantenendo sempre 3+3+3 slot
  -- distinti per ogni utente/stagione.
...
-- (9 slot totali: 3 moto3 + 3 moto2 + 3 motogp), senza sovrapposizioni di
...
-- Nota: l'admin compete anche lui in classifica (ha una sua rosa come
-- chiunque altro), quindi qui NON si filtra per ruolo, solo per attivo.
where u.attivo = true
...
-- ----------------------------------------------------------------------------
-- Restituisce esattamente i 9 piloti posseduti da un utente in un certo round,
